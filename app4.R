...
dataset <- read.csv("C:\\Users\\domin\\OneDrive\\Documents\\GitHub\\DDemarsico.github.io\\Data_Visualization_R\\Practicum_Cleaned_Data.csv")
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      menuItem("Instructions", tabName = "instructions_tab", icon = icon("info")),
  ...
  dashboardBody(
    tabItems(
      tabItem(tabName = "instructions_tab",
              h2("Instructions"),
              p("Welcome to the FND Data Explorer! This app is designed to allow anyone to easily explore the clinic data, regardless of programmin experience:"),
              tags$ul(
                tags$li("Step 1: Select the Exploration tab after reading these instructions"),
                tags$li("Step 2: Select either 1 or more variables to view/compare"),
                tags$li("Step 3: Stratify your data to create further comparisons")
              ),
              p("If you have any questions, please contact Dominick at: dominick.demarsico@cuanschutz.edu.")
      ),
      ...
      tabItem(tabName = "analysis_tab",
              ...
              fluidRow(
                box(title = "Analysis Section", width = 12, solidHeader = TRUE, status = "info",
                    p("This section will be for the Analysis Section."),
                    p("Planned build timme set for Summer, 2025")
                )
              )
      )        
    )
...
##############################################
  ...
  output$exploration_plot <- renderPlotly({
    ...
    if (input$secondary_var != "" && input$plot_type == "Boxplot") {
      vars_to_select <- c(input$primary_var, input$secondary_var)
      if (input$stratify && input$stratify_var != "") {
        vars_to_select <- c(vars_to_select, input$stratify_var)
      }
      
      df <- dataset %>%
        dplyr::select(all_of(vars_to_select)) %>%
        dplyr::filter(
          !is.na(.data[[input$primary_var]]),
          !is.na(.data[[input$secondary_var]]),
          if (input$stratify && input$stratify_var != "") !is.na(.data[[input$stratify_var]]) else TRUE
        )
      
      # If stratifying, create a combined group label
      if (input$stratify && input$stratify_var != "") {
        df$group_combined <- interaction(df[[input$secondary_var]], df[[input$stratify_var]], sep = " - ")
        
        p <- ggplot(df, aes(x = group_combined, y = .data[[input$primary_var]], fill = df[[input$stratify_var]])) +
          geom_boxplot(alpha = 0.7) +
          theme_minimal() +
          labs(
            x = paste(input$secondary_var, "+", input$stratify_var),
            y = input$primary_var,
            fill = input$stratify_var
          )
      } else {
        # No stratification
        p <- ggplot(df, aes_string(x = input$secondary_var, y = input$primary_var)) +
          geom_boxplot(fill = "#56B4E9", alpha = 0.7) +
          theme_minimal() +
          labs(x = input$secondary_var, y = input$primary_var)
      }
    ...
    if (input$secondary_var != "" && input$plot_type == "Scatterplot") {
      ...
      # Create base plot
      p <- ggplot(df, aes_string(x = input$primary_var, y = input$secondary_var))
      
      if (input$stratify && input$stratify_var != "") {
        # Scatterplot with color and separate trend lines by group
        p <- p +
          geom_point(aes_string(color = input$stratify_var), alpha = 0.7, size = 2) +
          geom_smooth(aes_string(color = input$stratify_var, group = input$stratify_var),
                      method = "lm") +
          labs(color = input$stratify_var)
      } else {
        # Scatterplot without stratification
        p <- p +
